...
是不是都有過類似的經驗？有時候為了避免樣式汙染到全域環境，我們會藉由 CSS 的子對象選擇器來完成指定，但此時你會發現大部分的時間都耗費在了父對象的撰寫上，不覺得這樣很浪費時間嗎？如果能在不修改 CSS 選擇器達到樣式名稱不重複效果該有多好呢？讓我們來看下個例子：
