...
- 直接提交至本地數據庫 (限以追蹤檔案)：`git commit -am 'message'`
- 比對 HEAD 與尚未進索引區檔案差異：`git diff`
- 比對 HEAD 與已進索引區檔案差異：`git diff --cached`
- 比對 HEAD 與已進索引區檔案差異 (同上)：`git diff --staged`
- 比對 HEAD 與 uncommitted 檔案差異：`git diff HEAD`
- 比對 uncommitted 與指定版本差異 (uncommitted)：`git diff HEAD~1`
- 比對 HEAD 與指定版本差異 (nothing to commit)：`git diff HEAD^`
- 比對兩個不同版本間差異 (舊版本 - 新版本)：`git diff HEAD^^ HEAD`
- 新增 difftool 選項 (VSCode 示範)：`git config --global diff.tool vscode`
- 設置 difftool 默認選項 (VSCode 示範)：`git config --global difftool.vscode.cmd 'code --wait --diff $LOCAL $REMOTE'`
